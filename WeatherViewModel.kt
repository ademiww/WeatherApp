...
class WeatherViewModel(
    ...
    private val _currentWeather = MutableLiveData<CurrentWeatherResponseDto?>()
    val currentWeather: LiveData<CurrentWeatherResponseDto?> get() = _currentWeather
    ...
    fun loadCurrentWeather(location: String) {
        viewModelScope.launch {
            try {
                _isLoading.value = true
                _currentWeather.value = getWeatherUseCase.execute(location)
            } catch (e: Exception) {
                _errorMessage.value = e.message
            } finally {
                _isLoading.value = false
